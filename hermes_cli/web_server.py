...
def _normalize_host_value(host_value: str) -> str:
    """Normalize a Host header or hostname for comparison."""
    h = host_value.strip()
    if not h:
        return ""
    if h.startswith("["):
...
def _parse_extra_allowed_hosts(raw_hosts: str | None) -> frozenset[str]:
    """Parse a comma-delimited list of extra trusted dashboard hosts."""
    if not raw_hosts:
        return frozenset()
    return frozenset(
        normalized_host
        for raw_host in raw_hosts.split(",")
        if (normalized_host := _normalize_host_value(raw_host))
    )
...
def _is_accepted_host(
    ...
    - Explicitly configured extra hostnames for trusted reverse proxies
    ...
    extra_hosts = extra_allowed_hosts or frozenset()
...
async def host_header_middleware(request: Request, call_next):
    ...
    if bound_host:
        ...
        extra_allowed_hosts = getattr(app.state, "extra_allowed_hosts", frozenset())
        if not _is_accepted_host(host_header, bound_host, extra_allowed_hosts):
            return JSONResponse(
                status_code=400,
                content={
                    "detail": (
...
def start_server(
    ...
    *,
    ...
    app.state.extra_allowed_hosts = _parse_extra_allowed_hosts(
        os.getenv("HERMES_EXTRA_ALLOWED_HOSTS")
    )
