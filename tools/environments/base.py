...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        bootstrap = (
            "if [ -n \"${_HERMES_ORIGINAL_PATH-}\" ]; then\n"
            "  __hermes_merge_path() {\n"
            "    local __hermes_list __hermes_entry\n"
            "    local __hermes_merged=\"\"\n"
            "    local __hermes_seen=\":\"\n"
            "    for __hermes_list in \"$@\"; do\n"
            "      [ -n \"$__hermes_list\" ] || continue\n"
            "      local IFS=':'\n"
            "      local __hermes_parts=()\n"
            "      read -r -a __hermes_parts <<< \"$__hermes_list\"\n"
            "      for __hermes_entry in \"${__hermes_parts[@]}\"; do\n"
            "        [ -n \"$__hermes_entry\" ] || continue\n"
            "        case \"$__hermes_seen\" in\n"
            "          *\":$__hermes_entry:\"*) continue ;;\n"
            "        esac\n"
            "        __hermes_seen=\"${__hermes_seen}${__hermes_entry}:\"\n"
            "        if [ -n \"$__hermes_merged\" ]; then\n"
            "          __hermes_merged=\"${__hermes_merged}:$__hermes_entry\"\n"
            "        else\n"
            "          __hermes_merged=\"$__hermes_entry\"\n"
            "        fi\n"
            "      done\n"
            "    done\n"
            "    printf '%s' \"$__hermes_merged\"\n"
            "  }\n"
            "  PATH=\"$(__hermes_merge_path \"${_HERMES_ORIGINAL_PATH}\" \"$PATH\")\"\n"
            "  export PATH\n"
            "  unset -f __hermes_merge_path\n"
            "  unset _HERMES_ORIGINAL_PATH\n"
            "fi\n"
